...
# Constants
DATA_PATH = 'C:\\Users\\axt43242\\Downloads\\irp-files-clean\\Extracted_Prod_Data.xlsx'
FOLDER_PATH = 'C:\\Users\\axt43242\\Downloads\\irp-files-clean\\Sands'
COMP_FILE_PATH = 'C:\\Users\\axt43242\\Downloads\\irp-files-clean\\Completion_Data.xlsx'
...
def select_decline_function(selected_function_name="Exponential Decline"):
    functions = {
        "Exponential Decline": (exponential_decline, ['b'], [1e-5]),
        "Hyperbolic Decline": (hyperbolic_decline, ['b', 'd'], [1e-5, 0.5]),
        "Harmonic Decline": (harmonic_decline, ['b'], [1e-5]),
        "Stretched Exponential Decline": (se_decline, ['tau', 'n'], [7, 0.1]),
        "Ilk Power Law Decline": (ilk_power_law, ['a', 'b', 'n'], [1e-5, 1e-5, 1e-3]),
        "Duong's Decline": (duong, ['a', 'm'], [1e-3, 1e-3])
    }
...
def train_and_fit(prd_df, merged_df, selected_function, initial_params, param_names):
    X, y = prepare_data_for_deep_learning(merged_df, ['BOEM_FIELD', 'LEASE', 'PLAY'], param_names)
    ...
    print("Training and evaluating XGBoost model with best hyperparameters...")
    ...
    for i, completion in enumerate(test_idx):
        try:
            ...
            elif isinstance(y_pred[i], np.float32):  # Handle single float value
    ...
    def plot_and_fit(group_name):
        with output:
            ...
            try:
                ...
                # Calculate R² and MSE for y_fit_curve
                mse_fit_curve = mean_squared_error(y_fit, y_fit_curve)
                r2_fit_curve = r2_score(y_fit, y_fit_curve)
                print(f"Mean Squared Error for {selected_function.__name__}: {mse_fit_curve}")
                print(f"R² Score for {selected_function.__name__}: {r2_fit_curve}")
            ...
            plt.ylabel('Daily Oil Rates in BOEPD')
            plt.legend()
            plt.tight_layout()
            plt.show()
...
def fit_decline_curves(prd_df, selected_function, initial_params, param_names):
    print("[DEBUG] fit_decline_curves started")
    unique_comps = prd_df['Well_Completion_Name'].unique()
    results = []

    for comp in unique_comps:
        group = prd_df[prd_df['Well_Completion_Name'] == comp]
        q0 = group['Daily_Rates_Oil'].max()
        max_q0_index = group['Daily_Rates_Oil'].idxmax()
...
def prepare_data_for_deep_learning(df, cat_cols, target_params):
    print(f"Initial dataframe shape: {df.shape}")
    print(f"Checking for missing values in the dataframe...")
    print(df.isnull().sum())
    ...
    # Ensure target columns do not have NaNs
    df.dropna(subset=target_params, inplace=True)

    # Check after dropping NaNs
    print(f"Dataframe shape after dropping NaNs in target columns: {df.shape}")

    # Drop target parameters from the features
    X = df.drop(columns=target_params)
    print(f'Shape of feature columns X: {X.shape}')
    
    y = df[target_params]
    print(f'Shape of target columns y: {y.shape}')
    ...
    if X.shape[0] != y.shape[0]:
        ...
        raise ValueError(f"Mismatch in the number of samples between X ({X.shape[0]}) and y ({y.shape[0]})")
